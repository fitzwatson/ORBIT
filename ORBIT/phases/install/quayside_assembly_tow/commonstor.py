...
__author__ = "Jake Nunemaker, adapted by Josh Watson"
__copyright__ = "Copyright 2020, National Renewable Energy Laboratory"
__maintainer__ = "Josh Watson"
__email__ = "joshuaw@umass.edu"
...
class SemiSubStorageInstallation:
    """Floating sub_storage Class."""

    def __init__(self):
        """Creates an instance of `SemiSubStorage`."""

        pass


class SemiSubStorageAssemblyLine(Agent):
    """Substructure Assembly Line Class."""

    def __init__(self, assigned, time, target, num):
        """
        Creates an instance of `SemiSubStorageAssemblyLine`.
        ...
        time : int | float
            Hours required to produce one sub_storage.
        target : simpy.Store
            Target storage.
        num : int
        ...
        super().__init__(f"SemiSubStorage Assembly Line {num}")
    ...
    @process
    def assemble_sub_storage(self):
        """
        Simulation process for assembling a sub_storage.
        """

        yield self.task("SemiSubStorage Assembly", self.time)
        sub_storage = sub_storage()

        start = self.env.now
        yield self.target.put(sub_storage)
    ...
    def start(self):
        ...
        while True:
            try:
                ...
                yield self.assemble_sub_storage()
...
class TurbineAssemblyLine(Agent):
    ...
    def __init__(self, feed, target, turbine, num):
        ...
        ----------
        feed : simpy.Store
            Storage for completed sub_storages.
        target : simpy.Store
            Target storage.
        num : int
            Assembly line number designation.
        ...
        while True:
            ...
            if delay > 0:
                self.submit_action_log(
                    "Delay: No sub_storages in Wet Storage", delay
                )
    ...
    @process
    def move_sub_storage(self):
        """
        Task representing time associated with moving the completed
        sub_storage assembly to the turbine assembly line.

        TODO: Move to dynamic process involving tow groups.
        """

        yield self.task("Move sub_storage", 8)

    @process
    def prepare_for_assembly(self):
        """
        Task representing time associated with preparing a sub_storage for
        turbine assembly.
        """
